(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))r(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const l of t.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&r(l)}).observe(document,{childList:!0,subtree:!0});function n(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function r(e){if(e.ep)return;e.ep=!0;const t=n(e);fetch(e.href,t)}})();const i=()=>{const o=["Surtu 2","Draft","Revisi"];return`
    <aside class="w-full md:w-[280px] bg-brand-panel border border-white/5 md:rounded-xl p-5 md:p-6 flex flex-col shrink-0 shadow-2xl">
      ...
      <nav id="sidebarNav" class="hidden md:flex flex-col gap-1.5">
        ${o.map(n=>`
      <a href="#" class="nav-item ${n==="Surtu 2"?"active bg-brand-accent-bg text-brand-accent font-semibold":"text-brand-text-muted hover:bg-white/5 hover:text-white"} px-4 py-3 rounded-lg text-base transition-all" data-menu="${n}">
        ${n}
      </a>
    `).join("")}
      </nav>
    </aside>
  `},a=()=>{const o=document.querySelectorAll(".nav-item");o.forEach(r=>{r.addEventListener("click",e=>{e.preventDefault(),o.forEach(t=>{t.classList.remove("active","bg-brand-accent-bg","text-brand-accent","font-semibold"),t.classList.add("text-brand-text-muted")}),r.classList.remove("text-brand-text-muted"),r.classList.add("active","bg-brand-accent-bg","text-brand-accent","font-semibold")})});const s=document.getElementById("mobileMenuBtn"),n=document.getElementById("sidebarNav");s&&n&&s.addEventListener("click",()=>{n.classList.toggle("hidden"),n.classList.toggle("flex")})},c=()=>`
...
(Cookie parsing logic goes here)`,n.value=l})},u=()=>`
  ...
  `,b=()=>{a(),d()};"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js",{scope:"/"}).then(o=>{console.log("SW registered: ",o)}).catch(o=>{console.log("SW registration failed: ",o)})});document.addEventListener("DOMContentLoaded",()=>{const o=document.getElementById("app");o&&(o.innerHTML=u(),b())});
