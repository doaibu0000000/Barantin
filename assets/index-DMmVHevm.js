...
(Cookie parsing logic goes here)`,n.value=e})},y=()=>`
  ...
  `,k=()=>{x(),w()},E="modulepreload",L=function(s,a){return new URL(s,a).href},b={},S=function(a,n,l){let e=Promise.resolve();if(n&&n.length>0){const i=document.getElementsByTagName("link"),t=document.querySelector("meta[property=csp-nonce]"),c=(t==null?void 0:t.nonce)||(t==null?void 0:t.getAttribute("nonce"));e=Promise.allSettled(n.map(d=>{if(d=L(d,l),d in b)return;b[d]=!0;const f=d.endsWith(".css"),h=f?'[rel="stylesheet"]':"";if(!!l)for(let m=i.length-1;m>=0;m--){const p=i[m];if(p.href===d&&(!f||p.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${d}"]${h}`))return;const u=document.createElement("link");if(u.rel=f?"stylesheet":E,f||(u.as="script"),u.crossOrigin="",u.href=d,c&&u.setAttribute("nonce",c),document.head.appendChild(u),f)return new Promise((m,p)=>{u.addEventListener("load",m),u.addEventListener("error",()=>p(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(i){const t=new Event("vite:preloadError",{cancelable:!0});if(t.payload=i,window.dispatchEvent(t),!t.defaultPrevented)throw i}return e.then(i=>{for(const t of i||[])t.status==="rejected"&&r(t.reason);return a().catch(r)})};function P(s={}){const{immediate:a=!1,onNeedReload:n,onNeedRefresh:l,onOfflineReady:e,onRegistered:r,onRegisteredSW:i,onRegisterError:t}=s;let c,d;const f=async(o=!0)=>{await d};async function h(){if("serviceWorker"in navigator){if(c=await S(async()=>{const{Workbox:o}=await import("./workbox-window.prod.es5-BqEJf4Xk.js");return{Workbox:o}},[],import.meta.url).then(({Workbox:o})=>new o("./sw.js",{scope:"./",type:"classic"})).catch(o=>{t==null||t(o)}),!c)return;c.addEventListener("activated",o=>{(o.isUpdate||o.isExternal)&&(n?n():window.location.reload())}),c.addEventListener("installed",o=>{o.isUpdate||e==null||e()}),c.register({immediate:a}).then(o=>{i?i("./sw.js",o):r==null||r(o)}).catch(o=>{t==null||t(o)})}}return d=h(),f}P({immediate:!0});document.addEventListener("DOMContentLoaded",()=>{const s=document.getElementById("app");s&&(s.innerHTML=y(),k())});
