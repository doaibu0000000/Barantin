...
--- MENCARI AJU: ${s} ---
`,u="",p="";const[v,n]=await Promise.all([k(s,"noAju"),k(s,"noReg")]);let b=v||n;if(b){if(p=b.ptk_id||b.id||"",b.noReg&&(u=b.noReg),p&&w)try{const y=await fetch(`https://api.karantinaindonesia.go.id/barantin-sys/ptk/${p}`,{headers:{Authorization:`Bearer ${w}`}});if(y.ok){const L=await y.json();(N=(A=L==null?void 0:L.data)==null?void 0:A.ptk)!=null&&N.no_dok_permohonan&&(u=L.data.ptk.no_dok_permohonan)}}catch(y){console.error("Failed to fetch PTK details",y)}let j="-",K="-",O="-",I=null;if(b.xml)try{I=JSON.parse(b.xml);const y=(S=(T=I==null?void 0:I.DOKUMEN)==null?void 0:T.ITEMS)==null?void 0:S.BARANG;if(y&&y.length>0){const L=y[0];j=L.URAIAN?L.URAIAN.trim():"-",K=`${L.NETTO||"-"} ${L.JNSSATUAN||""}`.trim(),O=`${L.JMLKEMAS||"-"} ${L.JNSKEMAS||""}`.trim(),y.length>1&&(j+=` (+${y.length-1} item lainnya)`)}}catch(y){console.error("Failed parsing xml",y)}let E="",R="";if(R+=`[DEBUG] Token      : ${w?"ADA":"KOSONG"}
`,R+=`[DEBUG] XML Parsed : ${I?"BERHASIL":"GAGAL/KOSONG"}
`,R+=`[DEBUG] Automasi   : OTOMATIS BERJALAN
...
`;try{const U=P(),V=new Date,ie=V.getTimezoneOffset()*6e4,q=new Date(V.getTime()-ie).toISOString().slice(0,19).replace("T"," "),re=q.split(" ")[0],se=((M=$.data)==null?void 0:M.nomor)||b.noReg||b.noAju,Z={id:U,ptk_id:D.id,no_dok_permohonan:se,ptk_analisis_id:"",nomor:"",tanggal:re+"T08:00",perihal:"Pelaksanaan Tindakan Karantina",penanda_tangan_id:2085,diterbitkan_di:"BANDUNG",user_id:(L==null?void 0:L.id)||"3267",created_at:q};R+=`[DEBUG] Surtug Req : ${JSON.stringify(Z)}
`;const z=await fetch("https://api3.karantinaindonesia.go.id/barantin-sys/surtug",{method:"POST",headers:{Authorization:`Bearer ${w}`,"Content-Type":"application/json"},body:JSON.stringify(Z)});if(z.ok||z.status===201){const H=await z.json();H.status==="201"||H.status===!0?E+=`Status Surtug  : BERHASIL DIBUAT (${((B=H.data)==null?void 0:B.nomor)||U})
...
`;return g});_=(await Promise.all(l)).join(""),e.value=_.trim(),F=_.trim(),a.disabled=!1,a.textContent="Proses Data",o&&o.classList.remove("hidden")}else e.value="Tidak ditemukan Nomor AJU SSM / PTK (26 karakter) yang valid pada teks.",F=e.value,o&&o.classList.add("hidden")})},ne=()=>`
...
Done`})},ue=()=>{let a=localStorage.getItem("activeMenu")||"Surtu 2";a==="Profile"&&(a="Surtu 2",localStorage.setItem("activeMenu","Surtu 2"));let t="";return a==="Draft"?t=ne():a==="Revisi"?t=ae():a==="Surtu 2"?t=te():t=`<div class="text-white text-center mt-10">Fitur ${a} belum tersedia</div>`,`
  ...
  `},me=()=>{const a=document.getElementById("mainContent");let t=localStorage.getItem("activeMenu")||"Surtu 2";t==="Profile"&&(t="Surtu 2"),le(e=>{localStorage.setItem("activeMenu",e),a&&(e==="Draft"?(a.innerHTML=ne(),Y()):e==="Revisi"?(a.innerHTML=ae(),Q()):e==="Surtu 2"?(a.innerHTML=te(),X()):a.innerHTML=`<div class="text-white text-center mt-10">Fitur ${e} belum tersedia</div>`)}),t==="Draft"?Y():t==="Revisi"?Q():t==="Surtu 2"&&X()},pe=()=>`
